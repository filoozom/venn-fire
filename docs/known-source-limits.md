...
| `cams-model-not-observation` | `inherent-source-limit` | CAMS wildfire PM10 and PM2.5 values are ensemble model grid forecasts, not local ground-sensor observations. They indicate possible smoke/particulate transport and never alter fire geometry. |
