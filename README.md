...
| Open-Meteo | Hourly model-grid temperature, feels-like temperature, humidity, precipitation probability/amount, weather code, cloud, visibility, wind and gust rows; recent overlap and at least the next 48 hours are retained | 5 min |
...
| DWD RADOLAN YW daily archive | Official 1 km, five-minute precipitation amounts from the ignition onward; each available compressed daily archive and every incident-area display frame are retained in PostgreSQL | 5 min publication check; 5 min observations |
...
The Best estimate uses one solid red boundary and one matching area figure. Its thermal core starts with high-confidence, independently corroborated VIIRS observations available by the selected time, then directly extends that same 50 m raster union with high-confidence pixels from the newest Terra/Aqua overpass that lie within 500 m of the core or aircraft-supported edge. Restricting MODIS to the newest supported pass prevents successive 1 km snapshots from accumulating into an inflated burn scar. Aircraft support is not a separate layer: repeated sharp `GRZLY##` direction changes must be within 1 km of the VIIRS core or its supported newest-pass MODIS edge and within 900 m of another five-minute evidence frame. This seed uses only observations available at the selected time, so later satellite corroboration never leaks backward. Each disconnected local cluster bounds its own smallest lobe against the thermal outline; the lobes are rasterised into the same union and therefore the same area figure. Approach, reservoir and disconnected route legs are excluded. Receiver data has no payload or drop-state field, so this remains a conservative inference rather than proof of a water drop or confirmed fire front. The separate touched-zone boundary remains removed.
