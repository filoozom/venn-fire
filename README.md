...
The Best estimate outline has three visibly separate components. Its solid red core remains the 50 m raster union of high-confidence, independently corroborated VIIRS footprints. Its dotted violet MODIS extension uses only the newest Terra/Aqua overpass available at each five-minute timeline frame, keeps high-confidence 1 km pixels within 500 m of the VIIRS core or aircraft-supported edge, and dissolves them on the same 50 m display grid. Restricting this component to the newest supported pass prevents successive coarse snapshots from accumulating into an inflated burn scar. Its dashed amber aircraft extension is derived at each five-minute frame from repeated sharp `GRZLY##` direction changes within 3.5 km of the core, requires nearby support from another five-minute frame, snaps the resulting evidence cells to the same 50 m grid and excludes long reservoir-side/transit route legs. Only the solid VIIRS core contributes to the hectare figure: a MODIS detection locates thermal activity only within its nominal 1 km pixel, while receiver data has no payload or drop-state field. Neither can establish a burned area or confirmed fire front.
...
- FIRMS pixels are thermal anomalies, not a burned-area perimeter. Independent sensors are never summed or averaged; only `providesArea: true` VIIRS products receive a footprint-union estimate. The latest supported high-confidence MODIS pass may expand the separately dotted Best estimate display, but MODIS and Meteosat never receive or change hectare figures.
